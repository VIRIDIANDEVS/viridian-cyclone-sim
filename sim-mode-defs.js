...
STORM_ALGORITHM.defaults.core = function(sys,u){
    ...
    if(!lnd && moisture >= 0.8) {
        sys.organization += sq(map(SST,20,26,28,29.5,31,0,0.5,0.75,1.4,2.1,true))*3*tropicalness;
    }
else if (!lnd && moisture >= 0.7 && moisture < 0.8) {
sys.organization += sq(map(SST,20,26,28,29.5,31,0,0.25,0.5,0.7,1.4,true))*3*tropicalness;
}
...
sys.organization += sq(map(SST,20,26,28,29.5,31,0,0.005,0.01,0.07,0.12,true))*3*tropicalness;
}
