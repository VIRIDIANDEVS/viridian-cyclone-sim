...
STORM_ALGORITHM.defaults.core = function(sys,u){
    ...
    if(!lnd) sys.organization += sq(map(SST,20,28,30,0,1,2,true))*3*tropicalness;
    ...
    let targetWind = map(sys.pressure,1013,900,25,160)*map(sys.lowerWarmCore,1,0,1,0.6);
...
STORM_ALGORITHM[SIM_MODE_EXPERIMENTAL].core = function(sys,u){
    ...
    sys.depth = lerp(sys.depth,lnd ? 0.5 : map(SST,26,28,32,0.5,1,4,true),tropicalness*sys.organization*0.025);
    ...
    if(sys.kaboom){
        if((!lnd || namedBoom) && (sys.organization > 0.8 || sys.kaboom === 2)){
            sys.kaboom = 2;
            if(sys.pressure > 600)
                sys.pressure -= random(5,10,15,20);
            sys.organization = 1;
            sys.lowerWarmCore = 1;
...
STORM_ALGORITHM.defaults.typeDetermination = function(sys,u){
    switch(sys.type){
        case TROP:
            ...
            break;
        case SUBTROP:
            sys.type = sys.lowerWarmCore<0.35 ? EXTROP : ((sys.organization<0.20 && sys.windSpeed<25) || sys.windSpeed<29) ? sys.upperWarmCore<0.25 ? EXTROP : TROPWAVE : sys.upperWarmCore<0.50 ? SUBTROP : TROP;
            break;
        case TROPWAVE:
